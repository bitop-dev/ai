...
func TestOpenAICompatibleChatStream(t *testing.T) {
	...
	client := CreateOpenAICompatible(Settings{
		APIKey:      "test-key",
		BaseURL:     server.URL,
		Headers:     map[string]string{"X-Custom": "custom"},
		QueryParams: map[string]string{"api-version": "2024-10-01"},
	})
...
func TestOpenAICompatibleResponsesStream(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	client := CreateOpenAICompatible(Settings{BaseURL: server.URL})
	...
	result, err := model.DoStream(context.Background(), provider.LanguageModelV3CallOptions{
		Prompt: prompt,
		ProviderOptions: provider.ProviderOptions{
			"openai-compatible": provider.JSONObject{"mode": "responses"},
		},
	})
	if err != nil {
		t.Fatalf("stream: %v", err)
