...
func TestCreateLangChainDefaults(t *testing.T) {
	client := CreateLangChain(Settings{})
...
func TestLangChainLanguageModelUsesEnvKey(t *testing.T) {
	t.Setenv("LANGCHAIN_API_KEY", "test-key")
	...
	model, err := client.LanguageModel("langchain-chat")
	if err != nil {
...
func TestLangChainToolCallStreaming(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		flusher := w.(http.Flusher)
		write := func(data string) {
	...
	client := CreateLangChain(Settings{BaseURL: server.URL})
	model, err := client.LanguageModel("langchain-tools")
	if err != nil {
		t.Fatalf("language model: %v", err)
	}
	prompt := provider.Prompt{
...
func TestLangChainUnsupportedModels(t *testing.T) {
	client := CreateLangChain(Settings{})
