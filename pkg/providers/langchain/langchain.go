...
const DefaultProviderName = "langchain"

// Settings configures the LangChain adapter.
...
// Provider wraps the OpenAI-compatible provider for LangChain endpoints.
...
// CreateLangChain constructs a LangChain adapter using OpenAI-compatible transport.
func CreateLangChain(settings Settings) *Provider {
...
func (p *Provider) EmbeddingModel(modelID provider.ModelID) (provider.EmbeddingModelV3, error) {
	return nil, provider.NewNoSuchModelError("langchain adapter does not support embedding models", nil, p.providerID, modelID)
}

func (p *Provider) ImageModel(modelID provider.ModelID) (provider.ImageModelV3, error) {
	return nil, provider.NewNoSuchModelError("langchain adapter does not support image models", nil, p.providerID, modelID)
}
